...
use quillai_log::{
    LogLevel, LogConfig, init_logger,
    info, debug, warn, error,
    info_span, debug_span, warn_span, create_span, Level, Instrument, Span
};
...
async fn demo_async_spans() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tasks = vec![
        process_async_task("task_1", "high", 80).instrument(info_span!("worker_1")),
        process_async_task("task_2", "medium", 120).instrument(info_span!("worker_2")),
        process_async_task("task_3", "low", 60).instrument(info_span!("worker_3")),
    ];
    ...
    async {
        ...
        let results = tokio::try_join!(tasks.0, tasks.1, tasks.2)?;
...
async fn demo_span_timing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let database_operation = info_span!("database_operation", table = "users", operation = "SELECT");
...
async fn demo_error_spans() -> Result<(), Box<dyn std::error::Error>> {
    ...
    async {
        ...
        let payment_span = info_span!("payment_attempt", transaction_id = "tx_789", amount = 150.00);
        let result = async {
