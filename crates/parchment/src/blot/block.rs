...
impl ParentBlotTrait for BlockBlot {
    ...
    fn insert_child_at_position(&mut self, position: usize, child: Box<dyn BlotTrait>) -> Result<(), JsValue> {
        ...
        // Note: BlockBlot's length() method already calculates length dynamically
...
impl FormattableBlot for BlockBlot {
    ...
    fn apply_format(&mut self, attributor: &dyn AttributorTrait, value: &JsValue) -> Result<(), JsValue> {
        ...
        if success {
            ...
            Err(JsValue::from_str(&format!(
                "Failed to apply format {} to BlockBlot",
                attributor.key_name()
            )))
        }
    }
